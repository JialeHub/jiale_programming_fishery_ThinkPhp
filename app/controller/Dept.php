...
class Dept
{
  public $allowKeys = ['name', 'sort', 'pid', 'status'];
   ...
   * @return Response
   * @OA\Get(
   *   path="/dept/list",summary="获取 | 部门列表（不分页）",tags={"Dept | 部门"},
  ...
  public function list(Request $request)
  {
    ...
    if ($request->has('tree')){
      ...
      if ($limit == -1){
        $list = $deptModel->withJOIN([
          'createUser' => $userShowField,
          'updateUser' => $userShowField
        ], 'left')->where($searchList)->order($sortList)->select();
        $count = count($list);
      }else{
        $count = $deptModel->withJOIN([
          'createUser' => $userShowField,
          'updateUser' => $userShowField
  ...
  public function export(Request $request)
  {
    ...
    $excelFileName = '部门数据导出_' . md5(json_encode($list, JSON_UNESCAPED_UNICODE)) . '.xlsx';
  ...
  public function update(Request $request): Response
  {
    $id = $request->param('id');
    $deptModel = new DeptModel;
    $dept = $deptModel->where('id', $id)->findOrEmpty();

    if ($dept->isEmpty()) {
      $status = 202;
      $msg = '找不到此记录';
    } else {
      $dept['updateId'] = $request->userId;
      $allowKeys = $this->allowKeys;;
      foreach ($allowKeys as $key) {
        if ($request->has($key, 'post')) $dept[$key] = $request->param($key);
      }
      $dept->save();
      $status = 200;
      $msg = '更新成功';
  ...
  public function delete(Request $request): Response
  {
    ...
    if (!is_null($id)) {
      ...
      $DeptModel = new DeptModel;
      $delList = $DeptModel->where('id', 'in', request()->param('ids'))->select();
      // 手动软删除
      $delList->update([
        'delete_id' => $deleteId,
        'delete_time' => date('Y-m-d H:i:s'),
